...
INJURIES = {"FEW": 12, "SEVERAL": 24, "DOZENS": 100, "HUNDREDS": 1000}
SHELTER = {"MINIMAL": 0.001, "CONSIDERABLE": 0.01}
...
LINKS = {
    "occupancy": "building_damage_occup.csv",
...
class HazusInfo(object):
    def __init__(self, version_folder, tract_csv, county_file, occupancy_file):
        self._countyfile = county_file
        self._tractfile = tract_csv
        self._occupancy_file = occupancy_file

        # parse the county level data file
        self._dataframe = pd.read_csv(county_file, dtype={"CountyFips": int})
    ...
    def createEconTable(self):
        ...
        for i in range(0, self._ncounties):
            ...
            loss_str = f"{int(lossvalue / 1000000.0):,}"
        ...
        total_str = f"{int(total_rounded / 1000000.0):,}"
    ...
    def createShelterTable(self):
        ...
        for i in range(0, self._ncounties):
            ...
            line = fmt % (county_name, state_abbrev, households, displaced, shelter)
            table_lines.append(line)
    ...
    def createDebrisTable(self):
        ...
        wood_total = f"{wood:.3f}"
        steel_total = f"{steel:.3f}"
        debris_total = f"{wood + steel:.3f}"
        table_lines = [
        ...
        table_lines.append(f"Brick / Wood & {wood_total} \\\\")
        table_lines.append(f"Reinforced Concrete / Steel & {steel_total} \\\\")
    ...
    def drawHazusMap(self, shakegrid, filename, model_config):
        ...
        if gd.xmin < gd.xmax:
            ...
            xmax += 360
            width = (
                ((gd.xmax + 360) - gd.xmin) * np.cos(np.radians(center_lat)) * 111.191
            )
        ...
        for cid, county in counties:
            ...
            county_name = (
                county["properties"]["NAMELSAD10"].replace("County", "").strip()
            )
        ...
        for county_shape in county_shapes:
            feature = ShapelyFeature(
                [county_shape], ccrs.PlateCarree(), zorder=COUNTY_ZORDER
            )
            ax.add_feature(
                feature,
                facecolor=GREY,
        ...
        for tid, tract in tracts:
            ...
            if econloss < 1e3:
                ...
                color = RED
            feature = ShapelyFeature(
                [tract_shape], ccrs.PlateCarree(), zorder=TRACT_ZORDER
            )
        ...
        t1 = time.time()
        logging.info(f"Done saving map - {t1 - t0:.2f} seconds")
    ...
    def create_tag_table(self, tag_dict, title):
        ...
        for key, value in tag_dict.items():
            line = f"{key} & {pop_round_short(value)} \\\\"
            table_lines.append(line)
        table_lines.append("\\end{tabularx}")
        table_text = "\n".join(table_lines)
        return table_text
