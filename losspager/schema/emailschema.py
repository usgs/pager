...
address_region_bridge = Table(
    "address_region_bridge",
    Base.metadata,
...
version_address_bridge = Table(
    "version_address_bridge",
    Base.metadata,
    Column(
        "version_id",
        Integer,
...
profile_region_bridge = Table(
    "profile_region_bridge",
    Base.metadata,
...
class User(Base):
    ...
    def fromDict(self, session, userdict):
        ...
        if reqfields <= set(userdict.keys()):
            ...
            raise PagerException(f"Missing required fields for user: {str(missing)}")
        # set the user fields
        ...
        org = (
            ...
            .filter(Organization.shortname == userdict["org"])
            .first()
        )
        if org is None:
            raise PagerException(
                f"No organization named {userdict['org']} exists in the database."
            )
...
class Address(Base):
    ...
    versions = relationship(
        "Version", secondary=version_address_bridge, back_populates="addresses"
    )
    ...
    def shouldAlert(
        self, version, renotify=False, release=False, ignore_time_limit=False
    ):
        ...
        if not ignore_time_limit:
            if datetime.utcnow() > version.time + timedelta(
                seconds=MAX_ELAPSED_SECONDS
            ):
    ...
    def fromDict(self, session, addressdict):
        ...
        if reqfields <= set(addressdict.keys()):
            ...
            raise PagerException(
                f"Missing required fields for address: {str(missing)}"
            )
        ...
        if not len(addressdict["profiles"]):
            logging.warning(
                "Warning: Address %s has NO profiles in the JSON file. Continuing."
                % self.email
            )
        for profiledict in addressdict["profiles"]:
            ...
            try:
                ...
                raise PagerException(
                    f'Error: "{str(pe)}" when loading profile for address {self.email}.'
                )
            if not len(profile.thresholds):
                logging.warning(
                    "Warning: Address %s has NO thresholds in one of the profiles. Continuing."
                    % self.email
                )
...
class Profile(Base):
    ...
    regions = relationship(
        "Region", secondary=profile_region_bridge, back_populates="profiles"
    )
    ...
    def __repr__(self):
        return "<Profile(%i thresholds,%i regions)>" % (
            len(self.thresholds),
            len(self.regions),
        )
    ...
    def fromDict(self, session, profiledict):
        ...
        if reqfields <= set(profiledict.keys()):
            ...
            raise PagerException(
                f"Missing required fields for profile: {str(missing)}"
            )
        ...
        for regiondict in profiledict["regions"]:
            ...
            if region is None:
                raise PagerException(
                    f"No region named {regiondict['name']} found in the database."
                )
...
class Event(Base):
    ...
    # An event can have many versions
    versions = relationship(
        "Version", back_populates="event", cascade="all, delete, delete-orphan"
    )

    def __repr__(self):
        return "<Event(eventcode='%s', %i versions)>" % (
            self.eventcode,
            len(self.versions),
        )
...
class Version(Base):
    ...
    addresses = relationship(
        "Address", secondary=version_address_bridge, back_populates="versions"
    )

    def __repr__(self):
        return "<Version(%s #%i, %s M%.1f)>" % (
            self.versioncode,
            self.number,
            str(self.time),
            self.magnitude,
...
class AlertScheme(Base):
    ...
    def __repr__(self):
        return f"<AlertScheme(name='{self.name}')>"

    def fromDict(self, session, schemedict):
        reqfields = set(
            ["name", "adesc", "valuetype", "isdiscrete", "minlevel", "maxlevel"]
        )
        if reqfields <= set(schemedict.keys()):
            ...
            raise PagerException(
                f"Missing required fields for alert scheme: {str(missing)}"
            )
        tscheme = AlertScheme(
            name=schemedict["name"],
...
class Threshold(Base):
    ...
    def __repr__(self):
        return f"<Threshold(type='{self.alertscheme.name}', value='{self.value}')>"
    ...
    def isMet(self, version, highest_level):
        ...
        """
        alertdict = {"green": 0, "yellow": 1, "orange": 2, "red": 3}
        levels = ["green", "yellow", "orange", "red"]
        ...
        if self.alertscheme.name == "eis":
            ...
            if highest_level < 0:
                ...
                two_levels_different = (
                    np.abs(version.summarylevel - highest_level) >= MAX_EIS_LEVELS
                )
    ...
    def fromDict(self, session, thresholddict):
        ...
        scheme = (
            session.query(AlertScheme)
            .filter(AlertScheme.name == thresholddict["alertscheme"])
            .first()
        )
        if scheme is None:
            raise PagerException(
                "No alert scheme named %s exists in the database."
                % thresholddict["alertscheme"]
            )
        if not scheme.isdiscrete:
            ...
            if tvalue < scheme.minlevel or tvalue > scheme.maxlevel:
                raise PagerException(f"Threshold for {scheme.name} is outside range.")
        self.alertscheme = scheme
        self.value = thresholddict["value"]

    def toDict(self):
        thresholddict = {"value": self.value, "alertscheme": self.alertscheme.name}
...
class RegionGroup(Base):
    ...
    def __repr__(self):
        return f"<RegionGroup({self.groupname})>"
...
class Region(Base):
    ...
    profiles = relationship(
        "Profile", secondary=profile_region_bridge, back_populates="regions"
    )

    regiongroup = relationship("RegionGroup", back_populates="regions")

    def __repr__(self):
        return f"<Region(name={self.name}, desc={self.desc})>"
    ...
    def fromDict(self, session, regiondict):
        ...
        if reqfields <= set(regiondict.keys()):
            ...
            raise PagerException(
                f"Missing required fields for region: {str(missing)}"
            )
        ...
        regiongroup = (
            session.query(RegionGroup)
            .filter(RegionGroup.groupname == rgroupname)
            .first()
        )
    ...
    def toDict(self):
        ...
        regiondict = {
            "type": "Feature",
            "geometry": polydata,
            "properties": {"code": regioncode, "desc": self.desc},
        }
        return regiondict
...
def create_db(url, schemadir, users_jsonfile=None, orgs_jsonfile=None):
    ...
    mmischeme = {
        "name": "mmi",
        "adesc": "Modified Mercalli Intensity",
        "valuetype": "Float",
        "isdiscrete": False,
        "minlevel": 0,
        "maxlevel": 10,
    ...
    # Level table
    levels = [
        {"ordernum": 0, "name": "green"},
        {"ordernum": 1, "name": "yellow"},
        {"ordernum": 2, "name": "orange"},
        {"ordernum": 3, "name": "red"},
    ]
    for level in levels:
        tlevel = Level(name=level["name"], ordernum=level["ordernum"])
