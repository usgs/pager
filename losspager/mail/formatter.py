...
# third party imports
from impactutils.textformat.text import (
    pop_round,
    dec_to_roman,
    pop_round_short,
    commify,
...
def format_city_table(cities):
    ...
    if len(cities):
        ...
        for idx, city in cities.iterrows():
            ...
            city_table += fmt.format(mmi=mmiroman, city=city["name"], pop=citypop)
...
def format_earthquakes(histquakes):
    ...
    for histquake in histquakes:
        ...
        mmistr = "{}({})".format(
            dec_to_roman(histquake["MaxMMI"]), pop_round_short(histquake["NumMaxMMI"])
        )
...
def format_short(version, expstr):
    ...
    if not version.released:
        alert_level = "pending"
    msg = f"""
    M{version.magnitude:.1f}
    D{int(version.depth):d}
    {version.time.strftime(DATE_TIME_FMT)}
    ({version.lat:.3f},{version.lon:.3f})
    ALERT:{alert_level.capitalize()}
    """
    msg = dedent(msg)
    msg += expstr
    return msg
...
def format_long(version, pdata, expstr, event_url, past_email_deadline=False):
    ...
    struct_comment = "\n".join(
        wrap(pdata.getStructureComment(), width=MAX_STRUCT_COMMENT_WIDTH)
    )
...
def format_msg(version, pdata, format, event_url, past_email_deadline=False):
    ...
    if format == "short":
        ...
        msg = format_long(
            version, pdata, expstr, event_url, past_email_deadline=past_email_deadline
        )
    return msg
